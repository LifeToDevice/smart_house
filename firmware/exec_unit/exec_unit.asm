...
;transceiver.h,34 :: 		memcpy(d, cmd, 5); //  ÓÔËÛÂÏ ÍÓÏ‡Ì‰Û ‚Ó ‚ÂÏÂÌÌ˚È ·ÛÙÂ
...
;transceiver.h,46 :: 		}
L_transceiver_send3:
;transceiver.h,48 :: 		COMM_DATA_DIR = 0;  // ¬˚ıÓ‰
	BCF         TRISB4_bit+0, BitPos(TRISB4_bit+0) 
...
;transceiver.h,72 :: 		memset(d, 0, 5); // ÔÓÒÎÂ Á‡ÔËÒË Ó˜Ë˘‡ÂÏ ·ÛÙÂ
	MOVLW       transceiver_send_d_L0+0
	...
	CALL        _memset+0, 0
;transceiver.h,76 :: 		COMM_DATA_DIR = 1;  // ¬ıÓ‰
	BSF         TRISB4_bit+0, BitPos(TRISB4_bit+0) 
;transceiver.h,77 :: 		COMM_WR_PIN   = 1;  // «‡ÔËÒ¸ Á‡ÔÂ˘ÂÌ‡
	BSF         RB7_bit+0, BitPos(RB7_bit+0) 
;transceiver.h,78 :: 		COMM_RD_PIN   = 0;
	BCF         RB6_bit+0, BitPos(RB6_bit+0) 
;transceiver.h,80 :: 		while ( COMM_DATA_READY_PIN ); // ∆‰ÂÏ „ÓÚÓ‚ÌÓÒÚË ˜ÚÂÌËˇ
...
;transceiver.h,91 :: 		d[n] <<= 1;             // —‰‚Ë„
	...
	MOVF        R0, 0 
	MOVWF       POSTINC1+0 
;transceiver.h,92 :: 		if ( COMM_DATA_PIN )    // ≈‰ËÌËˆ‡
...
;transceiver.h,100 :: 		GIE_bit = 1;
	BSF         GIE_bit+0, BitPos(GIE_bit+0) 
;transceiver.h,102 :: 		COMM_WR_PIN = 1; // «‡ÔËÒ¸ Á‡ÔÂ˘ÂÌ‡
	BSF         RB7_bit+0, BitPos(RB7_bit+0) 
;transceiver.h,103 :: 		COMM_RD_PIN = 1; // ◊ÚÂÌËÂ Á‡ÔÂ˘ÂÌÓ
	BSF         RB6_bit+0, BitPos(RB6_bit+0) 
...
;transceiver.h,123 :: 		COMM_DATA_DIR = 1;
	BSF         TRISB4_bit+0, BitPos(TRISB4_bit+0) 
;transceiver.h,125 :: 		COMM_WR_PIN  = 0;  // «‡ÔËÒ¸
	BCF         RB7_bit+0, BitPos(RB7_bit+0) 
;transceiver.h,126 :: 		COMM_RD_PIN  = 0;  //
	BCF         RB6_bit+0, BitPos(RB6_bit+0) 
;transceiver.h,127 :: 		COMM_CLK_PIN = 0;
	BCF         RB5_bit+0, BitPos(RB5_bit+0) 
;transceiver.h,129 :: 		Delay_ms(1); // ∆‰ÂÏ „ÓÚÓ‚ÌÓÒÚË Á‡ÔËÒË
	MOVLW       7
	MOVWF       R12, 0
	MOVLW       125
	MOVWF       R13, 0
L_transceiver_recv25:
	...
	DECFSZ      R12, 1, 1
	BRA         L_transceiver_recv25
;transceiver.h,131 :: 		if ( !COMM_DATA_READY_PIN ) // «‡ÔËÒ¸ ÓÚÍÎÓÌÂÌ‡
	BTFSC       RB3_bit+0, BitPos(RB3_bit+0) 
	GOTO        L_transceiver_recv26
;transceiver.h,133 :: 		COMM_WR_PIN = 1; // «‡ÔËÒ¸ Á‡ÔÂ˘ÂÌ‡
	BSF         RB7_bit+0, BitPos(RB7_bit+0) 
;transceiver.h,134 :: 		COMM_RD_PIN = 1; // «‡ÔËÒ¸ Á‡ÔÂ˘ÂÌ‡
	BSF         RB6_bit+0, BitPos(RB6_bit+0) 
...
;transceiver.h,147 :: 		d[n] <<= 1;           // —‰‚Ë„
	...
	MOVF        R0, 0 
	MOVWF       POSTINC1+0 
;transceiver.h,148 :: 		if ( COMM_DATA_PIN )  // ≈‰ËÌËˆ‡
	BTFSS       RB4_bit+0, BitPos(RB4_bit+0) 
...
;transceiver.h,156 :: 		GIE_bit = 1;
	BSF         GIE_bit+0, BitPos(GIE_bit+0) 
;transceiver.h,158 :: 		COMM_WR_PIN = 1; // «‡ÔËÒ¸ Á‡ÔÂ˘ÂÌ‡
	BSF         RB7_bit+0, BitPos(RB7_bit+0) 
;transceiver.h,159 :: 		COMM_RD_PIN = 1; // ◊ÚÂÌËÂ Á‡ÔÂ˘ÂÌÓ
	BSF         RB6_bit+0, BitPos(RB6_bit+0) 
...
;net.h,40 :: 		memset(res, 0, 5);
	...
	MOVWF       FARG_memset_n+1 
	CALL        _memset+0, 0
;net.h,42 :: 		d[0] = addr;  // ‡‰ÂÒ
	MOVF        FARG__net_send_addr+0, 0 
	MOVWF       _net_send_d_L0+0 
;net.h,43 :: 		d[1] = cmd;   // ÍÓÏ‡Ì‰‡
	MOVF        FARG__net_send_cmd+0, 0 
	MOVWF       _net_send_d_L0+1 
;net.h,44 :: 		d[2] = 0;     // ƒ‡ÌÌ˚Â (·‡ÈÚ 1)
	CLRF        _net_send_d_L0+2 
;net.h,45 :: 		d[3] = 0;     // ƒ‡ÌÌ˚Â (·‡ÈÚ 2)
	CLRF        _net_send_d_L0+3 
;net.h,46 :: 		d[4] = 0;     // ƒ‡ÌÌ˚Â (·‡ÈÚ 3)
	CLRF        _net_send_d_L0+4 
;net.h,48 :: 		if ( dat )    // ≈ÒÎË Ò ÍÓÏ‡Ì‰ÓÈ ÔÂÂ‰‡˛ÚÒˇ ‰‡ÌÌ˚Â
...
;net.h,76 :: 		if ( attempts )
	...
	BTFSC       STATUS+0, 2 
	GOTO        L_net_send45
;net.h,77 :: 		Delay_ms(100); // ÔÓ‰ÓÊ‰ÂÏ
	MOVLW       3
	MOVWF       R11, 0
	MOVLW       138
	MOVWF       R12, 0
...
;hc165.h,31 :: 		if ( HC165_DATA_PIN == 0 ) // »Ì‚ÂÚËÓ‚‡ÌÓ
...
;internal_eeprom.h,4 :: 		while ( WR_bit );                                                           // ≈ÒÎË Ë‰ÂÚ Á‡ÔËÒ¸, Ê‰ÂÏ
L_ee_read51:
	BTFSS       WR_bit+0, BitPos(WR_bit+0) 
	GOTO        L_ee_read52
	GOTO        L_ee_read51
L_ee_read52:
;internal_eeprom.h,5 :: 		EEADR     = addr;                                                           // ”ÒÚ‡Ì‡‚ÎË‚‡ÂÏ ‡‰ÂÒ
	MOVF        FARG_ee_read_addr+0, 0 
	MOVWF       EEADR+0 
;internal_eeprom.h,6 :: 		EEPGD_bit = 0;                                                              // Œ·‡˘ÂÌËÂ Í EEPROM
	BCF         EEPGD_bit+0, BitPos(EEPGD_bit+0) 
;internal_eeprom.h,7 :: 		CFGS_bit  = 0;                                                              // Œ·‡˘ÂÌËÂ Í EEPROM
	BCF         CFGS_bit+0, BitPos(CFGS_bit+0) 
;internal_eeprom.h,8 :: 		RD_bit    = 1;                                                              // «‡ÔÛÒÍ ˜ÚÂÌËˇ
	BSF         RD_bit+0, BitPos(RD_bit+0) 
;internal_eeprom.h,9 :: 		return EEDATA;                                                              // ¬ÓÁ‚‡˘‡ÂÏ ÁÌ‡˜ÂÌËÂ
	MOVF        EEDATA+0, 0 
	MOVWF       R0 
...
;internal_eeprom.h,15 :: 		char int_state = GIE_bit;                                                   // «‡ÔÓÏËÌ‡ÂÏ ÒÓÒÚÓˇÌËÂ ÔÂ˚‚‡ÌËÈ
	MOVLW       0
	BTFSC       GIE_bit+0, BitPos(GIE_bit+0) 
	MOVLW       1
	MOVWF       R2 
;internal_eeprom.h,16 :: 		while ( WR_bit );                                                           // ≈ÒÎË Ë‰ÂÚ Á‡ÔËÒ¸, Ê‰ÂÏ
L_ee_write53:
	BTFSS       WR_bit+0, BitPos(WR_bit+0) 
	GOTO        L_ee_write54
	GOTO        L_ee_write53
L_ee_write54:
;internal_eeprom.h,17 :: 		EEADR     = addr;                                                           // ”ÒÚ‡Ì‡‚ÎË‚‡ÂÏ ‡‰ÂÒ
	MOVF        FARG_ee_write_addr+0, 0 
	MOVWF       EEADR+0 
;internal_eeprom.h,18 :: 		EEDATA    = val;                                                            // ”ÒÚ‡Ì‡‚ÎË‚‡ÂÏ Á‡ÔËÒ˚‚‡ÂÏ˚Â ‰‡ÌÌ˚Â
	MOVF        FARG_ee_write_val+0, 0 
	MOVWF       EEDATA+0 
;internal_eeprom.h,19 :: 		EEPGD_bit = 0;                                                              // Œ·‡˘ÂÌËÂ Í EEPROM
	BCF         EEPGD_bit+0, BitPos(EEPGD_bit+0) 
;internal_eeprom.h,20 :: 		CFGS_bit  = 0;                                                              // Œ·‡˘ÂÌËÂ Í EEPROM
	BCF         CFGS_bit+0, BitPos(CFGS_bit+0) 
;internal_eeprom.h,21 :: 		WREN_bit  = 1;                                                              // –‡ÁÂ¯‡ÂÏ Á‡ÔËÒ¸
	BSF         WREN_bit+0, BitPos(WREN_bit+0) 
;internal_eeprom.h,22 :: 		GIE_bit   = 0;                                                              // «‡ÔÂ˘‡ÂÏ ÔÂ˚‚‡ÌËˇ
	BCF         GIE_bit+0, BitPos(GIE_bit+0) 
;internal_eeprom.h,23 :: 		EECON2    = 0x55;                                                           // ”ÒÚ‡Ì‡‚ÎË‚‡ÂÏ 0x55
	MOVLW       85
	MOVWF       EECON2+0 
;internal_eeprom.h,24 :: 		EECON2    = 0xAA;                                                           // ”ÒÚ‡Ì‡‚ÎË‚‡ÂÏ 0xAA
	MOVLW       170
	MOVWF       EECON2+0 
;internal_eeprom.h,25 :: 		WR_bit    = 1;                                                              // «‡ÔÛÒÍ Á‡ÔËÒË
	BSF         WR_bit+0, BitPos(WR_bit+0) 
;internal_eeprom.h,26 :: 		GIE_bit   = int_state;                                                      // ¬ÓÒÒÚ‡Ì‡‚ÎË‚‡ÂÏ ÒÓÒÚÓˇÌËÂ ÔÂ˚‚‡ÌËˇ
	BTFSC       R2, 0 
	GOTO        L__ee_write295
	BCF         GIE_bit+0, BitPos(GIE_bit+0) 
	GOTO        L__ee_write296
L__ee_write295:
	BSF         GIE_bit+0, BitPos(GIE_bit+0) 
L__ee_write296:
;internal_eeprom.h,27 :: 		WREN_bit  = 0;                                                              // «‡ÔÂ˘‡ÂÏ Á‡ÔËÒ¸
	BCF         WREN_bit+0, BitPos(WREN_bit+0) 
;internal_eeprom.h,28 :: 		while ( WR_bit );                                                           // ŒÊË‰‡ÂÏ ÓÍÓÌ˜‡ÌËˇ Á‡ÔËÒË
L_ee_write55:
	BTFSS       WR_bit+0, BitPos(WR_bit+0) 
	GOTO        L_ee_write56
	GOTO        L_ee_write55
L_ee_write56:
;internal_eeprom.h,29 :: 		return (WRERR_bit == 0);                                                    // ¬ÓÁ‚‡˘‡ÂÏ ÂÁÛÎ¸Ú‡Ú ‚˚‚ÓÎÌÂÌËˇ
	CLRF        R1 
...
;exec_unit.c,241 :: 		if ( byte_val == 0xff )  // –ü—Ä–æ–ø—É—Å–∫–∞–µ–º –ø—É—Å—Ç—ã–µ —è—á–µ–π–∫–∏
	MOVF        load_config_byte_val_L0+0, 0 
	XORLW       255
	BTFSS       STATUS+0, 2 
...
;exec_unit.c,280 :: 		while ( 1 )                                                                 // –ß–∏—Ç–∞–µ—Ç –∏–∑ –ø—Ä–∏–µ–º–æ–ø–µ—Ä–µ–¥–∞—Ç—á–∏–∫–∞, –ø–æ–∫–∞ –µ—Å—Ç—å –¥–∞–Ω–Ω—ã–µ
L_exec_commands122:
...
;exec_unit.c,283 :: 		memset(buff, 0, 5);
	...
	MOVWF       FARG_memset_n+1 
	CALL        _memset+0, 0
;exec_unit.c,284 :: 		if ( transceiver_recv(buff) )                                           // –ï—Å—Ç—å –¥–∞–Ω–Ω—ã–µ
	MOVLW       exec_commands_buff_L0+0
...
;exec_unit.c,286 :: 		is_connected = TRUE;                                                // –•–æ—Ç—è–±—ã –æ–¥–Ω–∞ –∫–æ–º–∞–Ω–¥–∞ –æ—Ç —Å–µ—Ä–≤–µ—Ä–∞ –±—ã–ª–∞ –ø—Ä–∏–Ω—è—Ç–∞
	MOVLW       1
	MOVWF       _is_connected+0 
;exec_unit.c,288 :: 		switch ( buff[1] )                                                  // –ö–æ–º–∞–Ω–¥–∞
...
L_exec_commands125:
;exec_unit.c,319 :: 		break;  // –î–∞–Ω–Ω—ã—Ö –±–æ–ª—å—à–µ –Ω–µ—Ç
...
;exec_unit.c,365 :: 		motors[i].sensor_state = (curr_sensors_state & (0x00400000 >> i)) ? 0 : 1;    // 0x00400000 - 23-–π –±–∞–π—Ç
	MOVLW       3
	MOVWF       R2 
...
;exec_unit.c,448 :: 		if ( !usb_state_on )   // –ü–æ–¥–∫–ª—é—á–µ–Ω USB –∫–∞–±–µ–ª—å, –≤–∫–ª—é—á–∞–µ–º USB HID
...
;exec_unit.c,456 :: 		if ( usb_state_on )    // –û—Ç–∫–ª—é—á–µ–Ω USB –∫–∞–±–µ–ª—å, –≤—ã–∫–ª—é—á–∞–µ–º USB HID
	MOVF        _usb_state_on+0, 1 
	BTFSC       STATUS+0, 2 
...
;exec_unit.c,465 :: 		if ( usb_read() )                                                       // –ü–æ–ª—É—á–∏–ª–∏ –∫–æ–º–∞–Ω–¥—É –æ—Ç –ü–ö
	CALL        _usb_read+0, 0
	MOVF        R0, 1 
	BTFSC       STATUS+0, 2 
...
;exec_unit.c,477 :: 		wr->cmd.name   = CMD_EU_GET_UNIT_NUMBER;
	...
	MOVLW       1
	MOVWF       POSTINC1+0 
;exec_unit.c,478 :: 		wr->number     = unit_number;                               // –ù–æ–º–µ—Ä –±–ª–æ–∫–∞
	MOVLW       2
...
;exec_unit.c,495 :: 		unit_number    = rd->number;                            // –ù–æ–º–µ—Ä –±–ª–æ–∫–∞
	MOVLW       2
	ADDWF       pc_data_exchange_rd_L4_L4+0, 0 
	MOVWF       FSR0 
	MOVLW       0
	...
	MOVF        POSTINC0+0, 0 
	MOVWF       _unit_number+0 
;exec_unit.c,496 :: 		wr->cmd.result = CMD_RESULT_OK;                         // –£—Å–ø–µ—à–Ω–æ
	MOVLW       1
	...
	MOVWF       FSR1H 
	CLRF        POSTINC1+0 
;exec_unit.c,497 :: 		reboot_needed  = TRUE;                                  // –¢—Ä–µ–±—É–µ—Ç—Å—è –ø–µ—Ä–µ–∑–∞–≥—Ä—É–∑–∫–∞
	MOVLW       1
	MOVWF       _reboot_needed+0 
;exec_unit.c,498 :: 		}
	GOTO        L_pc_data_exchange192
L_pc_data_exchange191:
;exec_unit.c,501 :: 		wr->cmd.result = CMD_RESULT_EEPROM_WRITE_FAIL;          // –û—à–∏–±–∫–∞ –ø—Ä–∏ –∑–∞–ø–∏—Å–∏ EEPROM
	MOVLW       1
	ADDWF       pc_data_exchange_wr_L4_L4+0, 0 
	MOVWF       FSR1 
	MOVLW       0
...
;exec_unit.c,506 :: 		wr->cmd.result = CMD_RESULT_BAD_INDEX;                       // –ù–µ–∫–æ—Ä—Ä–µ–∫—Ç–Ω—ã–π –Ω–æ–º–µ—Ä –±–ª–æ–∫–∞
	MOVLW       1
	ADDWF       pc_data_exchange_wr_L4_L4+0, 0 
	MOVWF       FSR1 
	MOVLW       0
...
;exec_unit.c,517 :: 		wr->cmd.name   = CMD_EU_GET_SENSORS_STATE;
	...
	MOVLW       3
	MOVWF       POSTINC1+0 
;exec_unit.c,518 :: 		wr->count      = SENSORS_COUNT - protected_motors;          // –ö–æ–ª–∏—á–µ—Å—Ç–≤–æ –¥–æ—Å—Ç—É–ø–Ω—ã—Ö –¥–∞—Ç—á–∏–∫–æ–≤
	MOVLW       2
	...
	MOVF        R0, 0 
	MOVWF       POSTINC1+0 
;exec_unit.c,519 :: 		wr->state      = sensors_state;                             // –°–æ—Å—Ç–æ—è–Ω–∏–µ –¥–∞—Ç—á–∏–∫–æ–≤
	MOVLW       3
	ADDWF       pc_data_exchange_wr_L4_L4_L4+0, 0 
	MOVWF       FSR1 
	MOVLW       0
...
;exec_unit.c,530 :: 		wr->cmd.name   = CMD_EU_GET_MOTORS_STATE;
	...
	MOVLW       4
	MOVWF       POSTINC1+0 
;exec_unit.c,531 :: 		wr->state      = (char)(motors_state & 0x000000ff);         // –°–æ—Å—Ç–æ—è–Ω–∏–µ –¥–≤–∏–≥–∞—Ç–µ–ª–µ–π (–≤–∫–ª/–≤—ã–∫–ª)
	MOVLW       2
	...
	MOVF        R0, 0 
	MOVWF       POSTINC1+0 
;exec_unit.c,532 :: 		wr->protect    = motor_get_protect_mask(protected_motors);  // –°–æ—Å—Ç–æ–Ω–∏–µ –∑–∞—â–∏—Ç—ã (–∏—Å–ø–æ–ª—å–∑—É–µ—Ç—Å—è/–Ω–µ –∏—Å–ø–æ–ª—å–∑—É–µ—Ç—Å—è)
	MOVLW       3
	ADDWF       pc_data_exchange_wr_L4_L4_L4_L4+0, 0 
...
;exec_unit.c,546 :: 		wr->number     = rd->number;
	...
	MOVF        POSTINC0+0, 0 
	MOVWF       POSTINC1+0 
;exec_unit.c,547 :: 		wr->state      = (motors_state & (1 << rd->number)) ? TRUE : FALSE;   // –í–∫–ª/–í—ã–∫–ª
	MOVLW       3
	ADDWF       pc_data_exchange_wr_L4_L4_L4_L4_L4+0, 0 
	MOVWF       R4 
	MOVLW       0
...
L_pc_data_exchange201:
	...
	MOVWF       POSTINC1+0 
;exec_unit.c,548 :: 		wr->protect    = motors[rd->number].protect;                          // –ò—Å–ø–æ–ª—å–∑—É–µ—Ç—Å—è/–ù–µ –∏—Å–ø–æ–ª—å–∑—É–µ—Ç—Å—è
	MOVLW       4
	ADDWF       pc_data_exchange_wr_L4_L4_L4_L4_L4+0, 0 
	MOVWF       FSR1 
	MOVLW       0
...
L__pc_data_exchange399:
	...
	MOVF        POSTINC0+0, 0 
	MOVWF       POSTINC1+0 
;exec_unit.c,549 :: 		wr->start_time = motors[rd->number].start_time / 10;                  // –í—Ä–µ–º—è —Ä–∞–∑–≥–æ–Ω–∞
	MOVLW       5
	ADDWF       pc_data_exchange_wr_L4_L4_L4_L4_L4+0, 0 
	MOVWF       FLOC__pc_data_exchange+0 
	MOVLW       0
...
;exec_unit.c,587 :: 		motor_stop(rd->number, FALSE);                       // –û—Å—Ç–∞–Ω–∞–≤–ª–∏–≤–∞–µ–º –¥–≤–∏–≥–∞—Ç–µ–ª—å
	MOVLW       2
	ADDWF       pc_data_exchange_rd_L4_L4_L4_L4_L4_L4+0, 0 
	MOVWF       FSR0 
	MOVLW       0
...
;exec_unit.c,591 :: 		motors[rd->number].protect = rd->protect;            // –ó–∞—â–∏—Ç–∞
	MOVLW       2
	ADDWF       pc_data_exchange_rd_L4_L4_L4_L4_L4_L4+0, 0 
	MOVWF       FSR0 
	MOVLW       0
...
L__pc_data_exchange407:
	...
	MOVF        POSTINC0+0, 0 
	MOVWF       POSTINC1+0 
;exec_unit.c,592 :: 		motors[rd->number].start_time = rd->start_time * 10; // –í—Ä–µ–º—è —Ä–∞–∑–≥–æ–Ω–∞
	MOVLW       2
	ADDWF       pc_data_exchange_rd_L4_L4_L4_L4_L4_L4+0, 0 
	MOVWF       FSR0 
	MOVLW       0
...
L__pc_data_exchange409:
	...
	MOVF        R1, 0 
	MOVWF       POSTINC1+0 
;exec_unit.c,594 :: 		protected_motors = motor_get_protected();            // –£—Å—Ç–∞–Ω–∞–≤–ª–∏–≤–∞–µ–º –∫–æ–ª–∏—á–µ—Å—Ç–≤–æ –∫–æ–Ω—Ç—Ä–æ–ª–∏—Ä—É–µ–º—ã—Ö –¥–≤–∏–≥–∞—Ç–µ–ª–µ–π
	CALL        _motor_get_protected+0, 0
	MOVF        R0, 0 
	MOVWF       _protected_motors+0 
...
;exec_unit.c,624 :: 		wr->state = (motors_state & (1 << rd->number)) ? TRUE : FALSE;  // –°–æ—Å—Ç–æ—è–Ω–∏–µ –¥–≤–∏–≥–∞—Ç–µ–ª—è
	MOVLW       3
...
;exec_unit.c,644 :: 		motor_start(rd->number);                            // –ó–∞–ø—É—Å–∫
	MOVLW       2
	ADDWF       pc_data_exchange_rd_L4_L4_L4_L4_L4_L4_L4_L4+0, 0 
	MOVWF       FSR0 
	MOVLW       0
...
;exec_unit.c,646 :: 		motor_stop(rd->number, FALSE);                      // –û—Å—Ç–∞–Ω–æ–≤–∫–∞
	MOVLW       2
	ADDWF       pc_data_exchange_rd_L4_L4_L4_L4_L4_L4_L4_L4+0, 0 
	MOVWF       FSR0 
	MOVLW       0
...
;exec_unit.c,663 :: 		wr->cmd.name   = CMD_EU_GET_UNIT_STATE;
	...
	MOVLW       9
	MOVWF       POSTINC1+0 
;exec_unit.c,664 :: 		wr->state      = is_connected;                              // –°–æ—Å—Ç–æ—è–Ω–∏–µ –±–ª–æ–∫–∞
	MOVLW       2
...
;exec_unit.c,702 :: 		wr->cmd.result = CMD_RESULT_OK;                 // –£—Å–ø–µ—à–Ω–æ
	MOVLW       1
	ADDWF       pc_data_exchange_wr_L4_L4_L4_L4_L4_L4_L4_L4_L4_L4+0, 0 
	MOVWF       FSR1 
	MOVLW       0
	...
	MOVWF       FSR1H 
	CLRF        POSTINC1+0 
;exec_unit.c,703 :: 		reboot_needed  = TRUE;                          // –¢—Ä–µ–±—É–µ—Ç—Å—è –ø–µ—Ä–µ–∑–∞–≥—Ä—É–∑–∫–∞
	MOVLW       1
	MOVWF       _reboot_needed+0 
;exec_unit.c,704 :: 		}
	GOTO        L_pc_data_exchange249
L_pc_data_exchange248:
;exec_unit.c,706 :: 		wr->cmd.result = CMD_RESULT_EEPROM_WRITE_FAIL;  // –û—à–∏–±–∫–∞ –ø—Ä–∏ –∑–∞–ø–∏—Å–∏ EEPROM
	MOVLW       1
	ADDWF       pc_data_exchange_wr_L4_L4_L4_L4_L4_L4_L4_L4_L4_L4+0, 0 
	MOVWF       FSR1 
	MOVLW       0
...
;exec_unit.c,742 :: 		wr->check_sum = crc16(wr->magic, 30);                       // –†–∞—Å—Å—á–∏—Ç—ã–≤–∞–µ–º –∫–æ–Ω—Ç—Ä–æ–ª—å–Ω—É—é —Å—É–º–º—É, –Ω–∞—á–∏–Ω–∞—è —Å magic
	MOVLW       32
	ADDWF       pc_data_exchange_wr_L4_L4_L4_L4_L4_L4_L4_L4_L4_L4_L4+0, 0 
	MOVWF       FLOC__pc_data_exchange+0 
	MOVLW       0
...
;exec_unit.c,758 :: 		usb_write();                                                        // –ü–µ—Ä–µ–¥–∞—á–∞ –∫–æ–º–∞–Ω–¥—ã –≤ –ü–ö
	CALL        _usb_write+0, 0
;exec_unit.c,759 :: 		}
...
;exec_unit.c,783 :: 		Delay_ms(1000);                                                             // –ñ–¥–µ–º –∏–Ω–∏—Ü–∏–∞–ª–∏–∑–∞—Ü–∏–∏ –æ—Å—Ç–∞–ª—å–Ω—ã—Ö –∫–æ–º–ø–æ–Ω–µ–Ω—Ç–æ–≤
	MOVLW       26
	MOVWF       R11, 0
	MOVLW       94
	MOVWF       R12, 0
...
L_main259:
	...
	BRA         L_main259
	NOP
;exec_unit.c,785 :: 		init();                                                                     // –û–±—â–∞—è –∏–Ω–∏—Ü–∏–∞–ª–∏–∑–∞—Ü–∏—è
	CALL        _init+0, 0
;exec_unit.c,787 :: 		transceiver_init();                                                         // –ü—Ä–∏–µ–º–æ–ø–µ—Ä–µ–¥–∞—Ç—á–∏–∫
	CALL        _transceiver_init+0, 0
;exec_unit.c,789 :: 		hc165_init();                                                               // HC165
	CALL        _hc165_init+0, 0
;exec_unit.c,791 :: 		motor_init();                                                               // –î–≤–∏–≥–∞—Ç–µ–ª–∏
	CALL        _motor_init+0, 0
;exec_unit.c,793 :: 		load_config();                                                              // –ó–∞–≥—Ä—É–∑–∫–∞ –∫–æ–Ω—Ñ–∏–≥—É—Ä–∞—Ü–∏–∏
	CALL        _load_config+0, 0
;exec_unit.c,795 :: 		timer1_init();
	CALL        _timer1_init+0, 0
;exec_unit.c,797 :: 		net_init(5);                                                                // –°–µ—Ç—å
	MOVLW       5
	MOVWF       FARG_net_init_send_attempts+0 
	CALL        _net_init+0, 0
;exec_unit.c,799 :: 		net_set_params(unit_number, 20);                                            // –£—Å—Ç–∞–Ω–æ–≤–∫–∞ –ø–∞—Ä–∞–º–µ—Ç—Ä–æ–≤ –ø—Ä–∏–µ–º–æ–ø–µ—Ä–µ–¥–∞—Ç—á–∏–∫–∞
	MOVF        _unit_number+0, 0 
...
;exec_unit.c,812 :: 		if ( net_send(SERVER_ADDRESS, NET_CMD_MOTORS_STATE, (uchar *)&motors_state, 0) == 0 )
	...
	BTFSS       STATUS+0, 2 
	GOTO        L_main265
;exec_unit.c,813 :: 		motor_reset_fail_state(); // –ï—Å–ª–∏ –∫–æ–º–∞–Ω–¥–∞ –æ —Å–æ—Å—Ç–æ—è–Ω–∏–∏ –¥–≤–∏–≥–∞—Ç–µ–ª–µ–π —É—Å–ø–µ—à–Ω–æ –æ—Ç–ø—Ä–∞–≤–ª–µ–Ω–∞, —Å–±—Ä–∞—Å—ã–≤–∞–µ–º –∞–≤–∞—Ä–∏–π–Ω–æ–µ —Å–æ—Å—Ç–æ—è–Ω–∏–µ –¥–≤–∏–≥–∞—Ç–µ–ª–µ–π
